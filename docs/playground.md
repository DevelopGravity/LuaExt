...
- **Host classes** — a panel for extending the bridge from the browser. Each entry is a PHP snippet that must `return` an object (registered with `registerObject()`, so its `#[LuaMethod]`-marked methods become visible), a class-string (registered with `registerClass()`, so instances cross into Lua as proxies — the shipped `Duration` entry demonstrates `.new`, statics, `tostring()` and mapped operators), or an `array<string, callable>` (registered with `registerLibrary()`):
