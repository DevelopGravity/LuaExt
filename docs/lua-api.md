...
- **`package` replacement**: exposes only `package.loaded` and `package.preload` (their *entries* writable the way upstream's are) and `package.path`. The table itself is read-only for real: it is a proxy whose `__newindex` raises, so `package.path = ...` or replacing `loaded`/`preload` wholesale is an error rather than a silent no-op — and none of it would change resolution anyway, which never reads the script's view. There is no `package.cpath`, `package.searchers`, or `package.loadlib` — nothing in this table can reach outside the sandbox.
...
- By-reference parameters cannot cross from Lua and are refused outright. Prefer-ref parameters (`current()`'s, `array_multisort()`'s) accept a plain value, exactly as the engine itself passes one. Enum-typed parameters are unsatisfiable from Lua (enums cross in neither direction) — accept the backing scalar and `::from()` it host-side.
...
A class the host passed to `registerClass()` reaches scripts as a **proxy**: an unforgeable full userdata wrapping one PHP object. `type(p)` is `"userdata"`, and there is no way to build one from Lua — a proxy only ever arrives from the host side.

- **Methods use the colon convention.** `p:method(...)` dispatches; the receiver is the wrapped object. Only methods the host exposed are visible — anything else reads as `nil` (`p.hidden == nil`), and calling a name that was never exposed fails the ordinary Lua way, catchably. `#[LuaMethod('other')]` renames a method for Lua without touching its PHP name.
- **Methods chain.** A method returning `$this`, a clone, or any other registered instance comes back as a proxy, so `p:add(2):add(3):total()` reads as it would in PHP. A subclass instance wrapping as its nearest registered ancestor still runs its own overrides.
- **Statics and the constructor live on a class table**, not on the instance: `Money.zero()`, `Money.parse("12.34")`, `Money.new(250)` — with a dot, because the table is not a receiver. Using a colon there (`Money:zero()`) is a catchable error whose message names the fix. The table is planted under the class's unqualified name unless `luaName:` says otherwise, and a class exposing only instance methods plants no table at all while still claiming its name.
- **`tostring(p)`** routes through `__toString` when the host exposed it, and otherwise gives Lua's ordinary `userdata: 0x...`.
- **Equality compares the wrapped objects, not the wrappers.** Two separate crossings of the same PHP object are `==`, even though they are distinct userdata and therefore *distinct table keys*. Comparing proxies of different classes, or a proxy against any other type, is simply `false` — the chain never raises.
- **Operators are opt-in per slot.** `#[LuaOperator(Operator::Add)]` (or the `operators:` map) backs `+`, `<`, `..` and the rest with a method. Both operands must be of the same registered class; a mixed or unmapped shape is a catchable error rather than a silent coercion. Lua derives `>` and `>=` itself by swapping the operands of `<` and `<=`.
- **Arguments obey the strict contract above**, on every door: methods, statics, `.new` and mapped operators alike.
- **A proxy is scoped to its sandbox** and to the registration that made it. After `unregister()`, instances a script already holds keep dispatching, while the class table and anything saved off it refuse — so a stale alias reports a withdrawn registration instead of reaching a reclaimed record.
- **Proxies cross coroutine boundaries freely**: one minted inside a coroutine and yielded out keeps working on the resuming side, and outlives the coroutine that made it.
