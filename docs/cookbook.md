...
`getOutput()` reads without clearing; `takeOutput()` reads and clears in one step — useful when a sandbox handles several `call()`s and you want per-call output rather than an ever-growing buffer. `stats()->outputTruncated` tells you whether any emitted output was lost — for either budget (`Limits::$outputBytes` or `Limits::$memoryBytes`), in either overflow mode.
