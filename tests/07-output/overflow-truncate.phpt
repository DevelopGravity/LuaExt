...
luaext
--XFAIL--
Needs print() from the library-policy wave. The cap, the truncation flag and its survival across takeOutput() are wired and verified from the C side; what is missing is a Lua-visible way to reach luaext_output_write() -- the base library is still upstream's, so print() writes to the process's stdout instead of to the sandbox.
--FILE--
<?php
